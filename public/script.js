...
if (!Array.prototype.find) {
  ...
  // iPad view elementer
  var ipadMessagesDiv = document.getElementById('ipad-messages');
  var ipadShoppingListUl = document.getElementById('ipad-shopping-list');
  var ipadMealPlanDiv = document.getElementById('ipad-meal-plan');
  var ipadUpcomingEventsDiv = document.getElementById('upcoming-events');
  ...
  socket.on('initialData', function(data) {
    ...
    updateShoppingList();
    updateCalendar();
    updateIpadView(); // Oppdater iPad-visningen også
  });
  
  // Funksjon for å oppdatere meldinger i iPad-visningen
  function updateIpadMessages() {
    ipadMessagesDiv.innerHTML = '';
    messages.forEach(function(msg) {
      var messageElem = document.createElement('div');
      messageElem.textContent = msg.text;
      ipadMessagesDiv.appendChild(messageElem);
    });
  }
  
  // Funksjon for å oppdatere handleliste i iPad-visningen
  function updateIpadShoppingList() {
    ipadShoppingListUl.innerHTML = '';
    shoppingList.forEach(function(item) {
      var itemElem = document.createElement('li');
      itemElem.textContent = item.text;
      ipadShoppingListUl.appendChild(itemElem);
    });
  }
  
  // Funksjon for å oppdatere ukens middagsplan i iPad-visningen
  function updateIpadMealPlan() {
    ipadMealPlanDiv.innerHTML = '';
    var mealPlan = { 
      monday: "Pasta med tomatsaus",
      tuesday: "Fiskekaker med poteter",
      wednesday: "Kyllinggryte",
      thursday: "Tacos",
      friday: "Pizza",
      saturday: "Grillet kylling",
      sunday: "Biff med grønnsaker"
    };
    Object.keys(mealPlan).forEach(day => {
      var dayElem = document.createElement('p');
      dayElem.textContent = `${day}: ${mealPlan[day]}`;
      ipadMealPlanDiv.appendChild(dayElem);
    });
  }
  
  // Funksjon for å oppdatere kommende hendelser i iPad-visningen
  function updateIpadUpcomingEvents() {
    ipadUpcomingEventsDiv.innerHTML = '';
    calendarEvents.forEach(function(event) {
      var eventElem = document.createElement('div');
      eventElem.textContent = event.text;
      ipadUpcomingEventsDiv.appendChild(eventElem);
    });
  }

  
  
  // Funksjon for å oppdatere hele iPad-visningen
  function updateIpadView() {
    updateIpadMessages();
    updateIpadShoppingList();
    updateIpadMealPlan();
    updateIpadUpcomingEvents();
  }
  ...
  socket.on('updateMessages', function(msgs) {
    messages = msgs;
    updateMessages();
    updateIpadMessages(); // Oppdater iPad-visningen for meldinger
  });
  ...
  socket.on('updateShoppingList', function(list) {
    shoppingList = list;
    updateShoppingList();
    updateIpadShoppingList(); // Oppdater iPad-visningen for handleliste
  });
  ...
  socket.on('updateCalendar', function(events) {
    ...
    updateCalendar();
    updateIpadUpcomingEvents(); // Oppdater iPad-visningen for kalenderhendelser
  });
  ...
  function updateClock() {
    ...
    const day = now.toLocaleDateString('no-NO', { weekday: 'long' });
    const fullDate = now.toLocaleDateString('no-NO', { day: 'numeric', month: 'long', year: 'numeric' });
